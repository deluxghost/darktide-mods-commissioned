...
local dataset = {
	...
	},
	{
		...
		display = "双子击杀数",
		force_default = false,
		format_func = format_func_template.integer,
		get_func = function (data, timer, timer_min)
			return data.twin_kills or 0
		end,
	},
}
...
for _, def in ipairs(defs.dataset) do
	if def.always then
		defs.dataset_always[def.name] = true
	end
end
